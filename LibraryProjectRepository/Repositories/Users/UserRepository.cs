...
namespace LibraryProjectRepository.Repositories.Users
{
    public class UserRepository : GinericRepository<User>
    {
        ...
        public UserRepository(LibraryDbContext context,
                              IMemoryCache cache, IConfiguration configuration) : base(context)
        {
        ...
        {
            // 1️⃣ تحقق من مطابقة كلمتي المرور
            if (user.Password != user.CaniformPassword)
                return null;

            // 2️⃣ تعيين الدور الافتراضي (User)
            var userRole = context.Roles.FirstOrDefault(r => r.Name == "User");
            if (userRole == null)
                return null; // أو ممكن تنشئه أثناء Seed
            ...
            var claims = new List<Claim>
        {
            ...
            // 6️⃣ إرجاع النتيجة النهائية (RegisterResult DTO)
